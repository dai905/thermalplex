...
boxsize = 3;
bg_filter = 20;
intensity_filter = 85; % set the intensity parameter to identify the spot
intensity_range = [200 300];
...
if ~isempty(puncta_location)
    showSpot(puncta_location(:,1:2),boxsize);
    writematrix(puncta_location,sprintf('single_cell_RNA_counts/%s_15_RNA_%s_488.txt',cell_name,Temperature))
    print(2,'-r300','-djpeg',sprintf('single_cell_RNA_counts/%s_15_RNA_%s_488.jpeg',cell_name,Temperature));
